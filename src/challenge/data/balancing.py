...
def balance_with_copula(
    ...
    if X_train.isna().any().any():
        raise ValueError(
            "X_train contains NaNs. Please impute missing values before balancing."
        )
    ...
    if num_to_synthesize <= 0:
        print(
            f"Data already meets target ratio ({n_minority}/{n_majority} >= {sampling_strategy}). No synthesis needed."
        )
        return X_train, y_train

    print(f"Balancing: {n_minority} minority, {n_majority} majority.")
    print(
        f"Target ratio: {sampling_strategy} -> Target minority count: {target_n_minority}"
    )
    ...
    # Shuffle
    data_resampled = data_resampled.sample(frac=1, random_state=42).reset_index(
        drop=True
    )
