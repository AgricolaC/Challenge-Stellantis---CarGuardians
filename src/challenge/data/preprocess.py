...
class ScaniaPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        na_drop_thresh=0.7,
        low_na_thresh=0.15,
        missing_flag_thresh=0.04,
        mice_estimator=Ridge(alpha=10),
        reduce_missingness=False,
        cluster_threshold=0.5,
    ):
        self.na_drop_thresh = na_drop_thresh
        self.low_na_thresh = low_na_thresh
        self.mice_estimator = mice_estimator
        self.max_iter = 1000  # Increased to ensure convergence
    ...
    def fit(self, X, y=None):
        ...
        if self.na_cols_to_drop_:
            print(
                f"  - Dropping {len(self.na_cols_to_drop_)} columns with > {self.na_drop_thresh:.0%} missing values: {sorted(list(self.na_cols_to_drop_))}"
            )
        if self.constant_cols_to_drop_:
            print(
                f"  - Dropping {len(self.constant_cols_to_drop_)} constant columns: {sorted(list(self.constant_cols_to_drop_))}"
            )
        ...
        self.low_na_cols_ = list(
            na_pct_filtered[
                (na_pct_filtered > 0) & (na_pct_filtered <= self.low_na_thresh)
            ].index
        )
        # High NA: > threshold (MICE)
        self.high_na_cols_ = list(
            na_pct_filtered[na_pct_filtered > self.low_na_thresh].index
        )
        ...
        if self.high_na_cols_:
            ...
            mice_pipe = Pipeline(
                [("scaler", StandardScaler()), ("estimator", self.mice_estimator)]
            )
        ...
        if self.reduce_missingness and self.missing_flag_cols_:
            ...
            try:
                ...
                cluster_labels = fcluster(
                    Z, t=self.cluster_threshold, criterion="distance"
                )
                ...
                print(
                    f"Clustering Missingness: Compressed {len(flag_cols)} _is_missing flags into {num_clusters} modules (Threshold={self.cluster_threshold})."
                )
                ...
                print(
                    f"Warning: Failed to cluster missingness flags. Skipping reduction. Error: {e}"
                )
    ...
    def transform(self, X, y=None):
        ...
        if self.missing_flag_cols_:
            ...
            missing_flags.columns = [
                f"{col}_is_missing" for col in self.missing_flag_cols_
            ]
            ...
            if self.reduce_missingness and self.missingness_cluster_map_:
                ...
                for i, cluster_cols in self.missingness_cluster_map_.items():
                    ...
                    if valid_cols:
                        ...
                        new_modules[f"Module_{i}_Connectivity"] = missing_flags[
                            valid_cols
                        ].max(axis=1)
        ...
        if self.median_imputer_ and self.low_na_cols_:
            X_transformed[self.low_na_cols_] = self.median_imputer_.transform(
                X_transformed[self.low_na_cols_]
            )
        # Apply MICE imputer
        if self.mice_imputer_ and self.high_na_cols_:
            X_transformed[self.high_na_cols_] = self.mice_imputer_.transform(
                X_transformed[self.high_na_cols_]
            )
        ...
        if X_transformed.isna().any().any():
            medians = pd.Series(
                self.fallback_imputer_.statistics_, index=self.columns_to_keep_
            )
