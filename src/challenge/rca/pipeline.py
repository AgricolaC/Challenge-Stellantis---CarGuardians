from .reference_model import train_reference_model, select_cost_threshold
from .explanation_shap import compute_shap_global, compute_shap_local
from .importance import compute_permutation_importance, compare_ranks
from .pdp_ice import generate_pdp_ice
from .surrogate import train_surrogate_tree, extract_rules
...
def run_rca_pipeline(
    ...
    print("\n--- 1) Train Reference Model (No Resampling) ---")
    model_ref, X_va, y_va, auc = train_reference_model(lgbm_weighted, X_dim_fixed, Y_fixed, random_state)
    
    # 2. Cost-Based Threshold Selection
    print("\n--- 2) Cost-Based Threshold Selection ---")
    threshold, min_cost = select_cost_threshold(model_ref, X_va, y_va, cost_fp, cost_fn, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    
    # 3. SHAP: Global & Local Explanations
    print("\n--- 3) SHAP: Global & Local Explanations ---")
    shap_values, X_shap, global_imp = compute_shap_global(model_ref, X_va, random_state=random_state, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    
    print("\n--- 3b) SHAP Local Explanations (Top 5 High Risk) ---")
    # Using threshold fro Step 2
    local_expl = compute_shap_local(model_ref, X_shap, shap_values, threshold, top_k=5, output_dir=output_dir, file_prefix=file_prefix)
    ...
    pi_df = compute_permutation_importance(model_ref, X_va, y_va, threshold, cost_fp, cost_fn, random_state=random_state, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    
    print("\n--- 4b) Compare Ranks ---")
    compare_ranks(global_imp, pi_df, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    
    # 5. PDP/ICE for Top Features
    print("\n--- 5) PDP/ICE for Top Features ---")
    # Take top 5 from SHAP
    top_features = global_imp["feature"].head(5).tolist()
    generate_pdp_ice(model_ref, X_va, top_features, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    ...
    surrogate_dt = train_surrogate_tree(model_ref, X_va, max_depth=3, random_state=random_state, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    extract_rules(surrogate_dt, X_va.columns, save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
    
    # 7. Decision Matrix & Radar Visualization
    print("\n--- 7) Decision Matrix & Radar Visualization ---")
    decision_matrix = create_decision_matrix(save_artifacts=True, output_dir=output_dir, file_prefix=file_prefix)
