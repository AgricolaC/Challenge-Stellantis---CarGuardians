...
def run_rca_pipeline(
    ...
    print("\n--- 2) Cost-Based Threshold Selection ---")
    threshold, min_cost = select_cost_threshold(model_ref, X_va, y_va, cost_fp, cost_fn, save_artifacts=True)
    
    # 3. SHAP: Global & Local Explanations
    print("\n--- 3) SHAP: Global & Local Explanations ---")
    shap_values, X_shap, global_imp = compute_shap_global(model_ref, X_va, random_state=random_state, save_artifacts=True)
    
    print("\n--- 3b) SHAP Local Explanations (Top 5 High Risk) ---")
    # Using threshold fro Step 2
    local_expl = compute_shap_local(model_ref, X_shap, shap_values, threshold, top_k=5)
    ...
    pi_df = compute_permutation_importance(model_ref, X_va, y_va, threshold, cost_fp, cost_fn, random_state=random_state, save_artifacts=True)
    
    print("\n--- 4b) Compare Ranks ---")
    compare_ranks(global_imp, pi_df, save_artifacts=True)
    
    # 5. PDP/ICE for Top Features
    print("\n--- 5) PDP/ICE for Top Features ---")
    # Take top 5 from SHAP
    top_features = global_imp["feature"].head(5).tolist()
    generate_pdp_ice(model_ref, X_va, top_features, save_artifacts=True)
    ...
    surrogate_dt = train_surrogate_tree(model_ref, X_va, max_depth=3, random_state=random_state, save_artifacts=True)
    extract_rules(surrogate_dt, X_va.columns, save_artifacts=True)
    
    # 7. Decision Matrix & Radar Visualization
    print("\n--- 7) Decision Matrix & Radar Visualization ---")
    decision_matrix = create_decision_matrix(save_artifacts=True)
    plot_radar_chart(decision_matrix)
    
    print("\n--- RCA Pipeline Complete! Artifacts are in current directory. ---")
