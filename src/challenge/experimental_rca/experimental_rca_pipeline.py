...
def run_pc_algorithm(X, y, features, feature_groups, rule_class_sink=True, rule_time_source=True, rule_mediation=True):
    ...
    for c in df.columns:
        if df[c].nunique() > 3:
            try:
                df[c] = pd.qcut(df[c], q=3, duplicates='drop', labels=False).fillna(0).astype(int)
            except ValueError:
                # Fallback
...
def run_lingam_algorithm(X, y, features, feature_groups, rule_class_sink=True, rule_time_source=True, rule_mediation=True, priors=None):
    ...
    print(f"DEBUG: Class Column (As Cause) Sum: {np.sum(np.abs(model.adjacency_matrix_[:, class_idx]))}")
    print(f"DEBUG: Class Row (As Effect) Sum: {np.sum(np.abs(model.adjacency_matrix_[class_idx, :]))}")
        ...
        for j in range(len(labels)): # j is Target
            ...
            if rule_class_sink and i == t_idx: 
                ...
                if j == time_idx and rule_mediation:
                    print(f"  [Physics] Pruned forbidden edge (Class -> Time): {labels[i]} -> {labels[j]}")
                    continue
            ...
            if rule_mediation and i == time_idx and j == t_idx:
                print(f"  [Physics] Pruned forbidden edge: {labels[i]} -> {labels[j]} (Weight: {weight:.4f})")
                continue 
...
def classify_causal_nodes(adj, labels, target='class', time_node='aa_000'):
    ...
    """
    G = nx.DiGraph()
    for i, lbl in enumerate(labels): G.add_node(lbl)
    ...
    for node in labels:
        if node == target or node == time_node: continue
    ...
    for i in range(len(features)):
        for j in range(i+1, len(features)):
            ...
            if corr.loc[f1, f2] > 0.80:
                c1 = abs(df_feat[f1].corr(y)); c2 = abs(df_feat[f2].corr(y))
...
def run_pymc_age_adjusted(X, y, features, name, control_feat='aa_000'):
    ...
    for feat in features:
        ...
        with pm.Model() as model:
            ...
            trace = pm.sample(600, tune=600, target_accept=0.95, progressbar=False, cores=1)
...
def rank_features_age_adjusted(X, y, features, control_feat='aa_000'):
    ...
    if not df_results.empty:
        ...
        df_results = df_results.sort_values(by='Coeff (LogOdds)', key=abs, ascending=False)
        
    return df_results
    


def create_hybrid_graph(lingam_adj, pc_adj, labels_lingam, labels_pc, target='class', time_node='aa_000', rule_time_source=True):
    print("\n--- Creating Hybrid Ensemble Graph (LiNGAM + PC) ---")
    G = nx.DiGraph()
    # Use LiNGAM labels as the base set of nodes
    for lbl in labels_lingam: G.add_node(lbl)
...
def visualize_hybrid(G, filename="rca_hybrid_graph.png"):
    ...
    node_colors = []
    for n in G.nodes():
        if n == 'class': node_colors.append('red')
        elif n == 'aa_000': node_colors.append('green')
        else: node_colors.append('skyblue')
        
    nx.draw(G, pos, node_color=node_colors, edge_color=colors, width=weights, 
            with_labels=True, node_size=2500, font_size=10, arrowsize=20)
            
    # Legend
    from matplotlib.lines import Line2D
    legend_lines = [Line2D([0], [0], color='black', lw=2, label='LiNGAM Edge (Strong)'),
...
def visualize_hierarchy_final(G, title, filename, pymc_results=None):
    ...
    if pymc_results is not None:
        ...
        if 'Algo' in current_pymc_results.columns:
            if 'PC' in title:
                current_pymc_results = current_pymc_results[current_pymc_results['Algo'] == 'PC']
            elif 'LiNGAM' in title:
                current_pymc_results = current_pymc_results[current_pymc_results['Algo'] == 'LiNGAM']
    # ---------------------------------------------------
    ...
    for n in G.nodes():
        ...
        if current_pymc_results is not None and not current_pymc_results.empty and 'Feature' in current_pymc_results.columns:
...
def merge_results(output_dir, experiments, n_features):
    ...
    if merged_nodes:
        ...
        final_df.to_csv(f"{output_dir}/rca_experiment_comparison_nodes_n={n_features}.csv", index=False)
        print(f"Saved Merged Node Results: {output_dir}/rca_experiment_comparison_nodes_n={n_features}.csv")
...
def main():
    ...
    X_cont = pd.DataFrame(scaler.fit_transform(X_transformed), columns=X_transformed.columns, index=X_transformed.index)
    X_bal_cont, _ = balance_for_structure_learning(X_cont, y_raw, ratio=1.0) # For LiNGAM (Continuous)
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)# Compare: No Rules vs Class Rules vs Full Physics
    ...
    for exp_name, r_sink, r_source, r_med in experiments:
        ...
        print(f"Rules -> Class Sink: {r_sink}, Time Source: {r_source}, Mediation: {r_med}")
        print(f"========================================")
        
        # Balance Data (Once per experiment to ensure fairness, or re-sample)
        X_bal, y_bal = balance_for_structure_learning(X_transformed, y_raw, ratio=1.0) # Changed y_clean to y_raw
        ...
        if 'PyMC_Mean' not in df_combined.columns: df_combined['PyMC_Mean'] = np.nan
        if 'PyMC_Odds_Ratio' not in df_combined.columns: df_combined['PyMC_Odds_Ratio'] = np.nan
        ...
        if r_source:
            ...
            if not r_med and 'aa_000' in final_features:
                try:
                    ...
                    if screen_results is None: screen_results = pd.DataFrame([row_age])
                    else: screen_results = pd.concat([screen_results, pd.DataFrame([row_age])], ignore_index=True)
                    
                    # Re-sort
                    screen_results = screen_results.sort_values(by='Coeff (LogOdds)', key=abs, ascending=False)
            ...
            if candidates and screen_results is not None and not screen_results.empty:
                screen_subset = screen_results[screen_results['Feature'].isin(candidates)].head(5)
                
                # 2. Deep Dive PyMC on top candidates
                top_candidates = screen_subset['Feature'].tolist()[:3]
                if top_candidates:
                    print(f"Running Deep Dive PyMC on Top 3 Candidates: {top_candidates}")
                    pymc_deep_dive_df = run_pymc_age_adjusted(X_cont, y_raw, top_candidates, "Top Candidates", control_feat='aa_000')
                    
                    if pymc_deep_dive_df is not None and not pymc_deep_dive_df.empty:
                        for feat in top_candidates:
                            row = pymc_deep_dive_df[pymc_deep_dive_df['Feature'] == feat]
                            if not row.empty:
                                res = row.iloc[0]
                                mask = (df_combined['Feature'] == feat) & (df_combined['Experiment'] == exp_name)
                                df_combined.loc[mask, 'PyMC_Mean'] = res['Mean']
                                df_combined.loc[mask, 'PyMC_Odds_Ratio'] = res['Odds_Ratio']
        else:
            # --- Default Validation (Standard PyMC) ---
            if candidates:
                # Run standard PyMC on candidates
                summary, valid_feats = run_pymc_validation(X_cont, y_raw, candidates, "Candidates")
                ...
                if summary is not None and not summary.empty:
                    ...
                    for i, feat in enumerate(valid_feats):
                        try:
                            ...
                            pymc_results_list.append({'Feature': feat, 'Mean': mean_val})
        ...
        visualize_hierarchy_final(G_lingam, f"LiNGAM - {exp_name}", f"{output_dir}/rca_lingam_{exp_name}_n={n_features}.png", screen_results_for_viz)
        visualize_hierarchy_final(G_pc, f"PC - {exp_name}", f"{output_dir}/rca_pc_{exp_name}_n={n_features}.png", screen_results_for_viz)

        # --- HYBRID GRAPH (Ensemble) ---
        # Run for ALL experiments as requested
        G_hybrid = create_hybrid_graph(adj_lingam, adj_pc, labels_lingam, labels_pc, rule_time_source=r_source) # Pass rule_time_source
        analyze_hybrid_layers(G_hybrid)
        visualize_hybrid(G_hybrid, f"{output_dir}/rca_hybrid_{exp_name}_n={n_features}.png")
        
        # Export Hybrid Edges
        hybrid_edges = []
        for u, v, d in G_hybrid.edges(data=True):
            hybrid_edges.append({'Algo': 'Hybrid', 'Source': u, 'Target': v, 'Type': d.get('type', 'Unknown')})
        pd.DataFrame(hybrid_edges).to_csv(f"{output_dir}/rca_edges_hybrid_{exp_name}_n={n_features}.csv", index=False)

        # Export Edges (per experiment)
        edges_data = []
        for u, v in G_pc.edges(): edges_data.append({'Algo': 'PC', 'Source': u, 'Target': v})
        for u, v in G_lingam.edges(): edges_data.append({'Algo': 'LiNGAM', 'Source': u, 'Target': v})
        pd.DataFrame(edges_data).to_csv(f"{output_dir}/rca_edges_{exp_name}_n={n_features}.csv", index=False)

        df_combined.to_csv(f"{output_dir}/rca_node_details_{exp_name}_n={n_features}.csv", index=False)
...
def consolidate_results(output_dir, n_features):
    ...
    node_files = [f for f in os.listdir(output_dir) if f.startswith('rca_node_details_') and f.endswith(f'_n={n_features}.csv')]
    merged_nodes = []
    for f in node_files:
        try:
            ...
            if 'Experiment' not in df.columns:
                exp_name = f.replace('rca_node_details_', '').replace(f'_n={n_features}.csv', '')
                df['Experiment'] = exp_name
            merged_nodes.append(df)
        except Exception as e:
    ...
    edge_files = [f for f in os.listdir(output_dir) if f.startswith('rca_edges_') and f.endswith(f'_n={n_features}.csv')]
    merged_edges = []
    for f in edge_files:
        try:
            df = pd.read_csv(os.path.join(output_dir, f))
