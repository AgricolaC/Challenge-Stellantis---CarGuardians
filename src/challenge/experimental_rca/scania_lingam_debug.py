...
def collapse_histograms(df):
    ...
    keep_cols = [
        c
        for c in df.columns
        if not any(c.startswith(f + "_") and c[-1].isdigit() for f in families)
    ]
    df_new = df[keep_cols].copy()

    for fam in families:
        bins = sorted(
            [c for c in df.columns if c.startswith(fam + "_") and c[-1].isdigit()]
        )
        if not bins:
            continue
...
def configure_debug_priors(labels, enforce_mediation=False):
    ...
    try:
        ...
        if enforce_mediation:
            ...
            print(
                f"  Constraint: Time -> Class is FORBIDDEN (priors[{t_idx}, {time_idx}] = -1)"
            )
        else:
            print(
                f"  Constraint: Time -> Class is ALLOWED (priors[{t_idx}, {time_idx}] = 0)"
            )
...
def run_debug():
    ...
    for src, tgt, w in edges:
        mark = ""
        if src == "aa_000" and tgt == "class":
            mark = " <--- DIRECT TIME LINK"
        if tgt == "class":
            mark += " (To Class)"
