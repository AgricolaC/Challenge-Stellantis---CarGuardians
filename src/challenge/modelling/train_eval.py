...
def _process_fold(
    ...
    Refactored for Ironclad Protocol:
    1. Impute (Train fit)
    2. Select Features (Train fit - No Leakage)
    3. Balance (Train only)
    4. Scale (Train fit)
    5. Train
    """
    # 1. Slice Data
    ...
    # --- 2. Impute (Golden Pipeline Step 1) ---
    ...
    # --- 3. Feature Selection (INSIDE LOOP = NO LEAKAGE) ---
    # Default to all columns if no selector provided
    selected_features = X_tr_imputed.columns.tolist()

    if feature_selector is not None:
        # Pass y_tr to selector
        # Note: Selector functions in feature_selection.py return a list of strings
        selected_features = feature_selector(X_tr_imputed, y_tr, n_features=n_features)

        # Apply selection to Imputed Data
        X_tr_imputed = X_tr_imputed[selected_features]
        X_va_imputed = X_va_imputed[selected_features]

    # --- 4. Balancing (SMOTE/Copula) - WITH COLUMN NAME FIX ---
    ...
    if sampler is not None:
        ...
        if sampler == "copula":
            strategy = (
                sampling_strategy if isinstance(sampling_strategy, float) else 1.0
            )
            X_tr_balanced, y_tr_balanced = balance_with_copula(
                X_tr_imputed, y_tr, sampling_strategy=strategy
            )
        else:
            # SMOTE returns numpy; reconstruct DataFrame to keep feature names safe
            X_res, y_res = samplers[sampler].fit_resample(X_tr_imputed, y_tr)
            X_tr_balanced = pd.DataFrame(
                X_res, columns=selected_features
            )  # <--- CRITICAL FIX
            y_tr_balanced = y_res
    ...
    X_tr_scaled = pd.DataFrame(
        scaler.fit_transform(X_tr_balanced),
        columns=selected_features,
        index=X_tr_balanced.index,
    )
    X_va_scaled = pd.DataFrame(
        scaler.transform(X_va_imputed),
        columns=selected_features,
        index=X_va_imputed.index,
    )
    ...
    if tune_threshold:
        # OPTIMIZATION: Sort approach O(N log N) vs Grid Search O(N*K)
        # Using existing optimized grid search for safety and proven correctness
        best_thr, best_cost = _best_threshold_by_cost(
            y_va, prob, cost_fp, cost_fn, max_threshold
        )
    ...
    return {
        ...
        "thr": best_thr,
        "f1": f1,
        "selected_features": selected_features,  # Feature stability tracking
        "fit_time": fit_t,
        "pred_time": pred_t,
    }
...
def cv_cost(
    ...
    *,
    ...
    """
    Cross-validates the full "Impute -> Select -> Balance -> Scale -> Train" pipeline.
    """
    y = _to_array(y)
    ...
    selected_features_folds = [
        r["selected_features"] for r in results
    ]  # <--- Collect Features
    ...
    out = {
        ...
        "selected_features": selected_features_folds,  # <--- Store for Stability Analysis
    }
    if return_threshold:
    ...
    *,
    ...
    if threshold is None and tune_if_none:
        ...
        if sampler is not None:
            ...
            if sampler == "copula":
                ...
                X_tr_balanced, y_tr_balanced = samplers[sampler].fit_resample(
                    X_tr_imputed, y_tr
                )
        ...
        if hasattr(X_tr_balanced, "columns"):
            X_tr_scaled = pd.DataFrame(
                _X_tr_scaled_np,
                columns=X_tr_balanced.columns,
                index=X_tr_balanced.index,
            )
            X_va_scaled = pd.DataFrame(
                _X_va_scaled_np, columns=X_tr_balanced.columns, index=X_va_imputed.index
            )
        else:
        ...
        threshold, tune_cost = _best_threshold_by_cost(
            y_va, prob_val, cost_fp, cost_fn, max_threshold
        )
        if verbose:
            print(
                f"Threshold tuned in {fmt_secs(fit_sel)} -> thr={threshold:.3f} (Val Cost={tune_cost:.0f})"
            )
    ...
    if sampler is not None:
        ...
        if sampler == "copula":
            ...
            X_train_balanced, y_train_balanced = balance_with_copula(
                X_train_imputed, y_train, sampling_strategy=strategy
            )
        else:
            X_train_balanced, y_train_balanced = samplers[sampler].fit_resample(
    ...
    if verbose:
        ...
        print(
            f"Test CM (thr={thr:.3f}): FP={metrics['FP']:.0f}, FN={metrics['FN']:.0f}"
        )
