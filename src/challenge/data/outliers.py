...
def visualize_outlier_rescue(X, y, contamination=0.01, filename="outlier_rescue_diagram.png"):
    """
    ...
    is_true_noise = (is_outlier_raw) & (y == 0) # Orange (Healthy but Anomalous -> Noise)
    ...
    plt.subplot(1, 2, 1)
    plt.title("The Mechanism: Isolation Forest Detection", fontsize=16, fontweight='bold')
    
    # Plot Inliers (Blue Cloud)
    plt.scatter(X_viz[is_inlier, 0], X_viz[is_inlier, 1], c='#3498db', alpha=0.3, s=10, label='Healthy Inliers (Cluster)')
    
    # Plot Raw Outliers (Orange/Red) - Both types mixed initially
    plt.scatter(X_viz[is_true_noise, 0], X_viz[is_true_noise, 1], c='#e67e22', marker='x', s=40, label='Raw Outliers (Noise)')
    plt.scatter(X_viz[is_rescued, 0], X_viz[is_rescued, 1], c='#e67e22', marker='x', s=40) # Same color initially
    ...
    # Kept Healthy (Blue)
    plt.scatter(X_viz[is_inlier, 0], X_viz[is_inlier, 1], c='#3498db', alpha=0.1, s=10, label='Kept: Healthy')
    
    # Dropped Noise (Orange)
    plt.scatter(X_viz[is_true_noise, 0], X_viz[is_true_noise, 1], c='#e67e22', marker='x', alpha=0.2, s=30, label='Dropped: Noise (Healthy Outliers)')
    
    # Rescued Failures (Green) - The Hero
    plt.scatter(X_viz[is_rescued, 0], X_viz[is_rescued, 1], c='#2ecc71', marker='*', s=150, edgecolors='black', label='RESCUED: Failure Outliers')
    
    # Annotation
    n_rescued = sum(is_rescued)
    plt.text(0.05, 0.95, f"Safety Gate Logic:\nmask[y == 1] = True\n\nRescued {n_rescued} data points", 
...
def fit_predict_isolation_forest(X, y=None, contamination=0.01, random_state=42):
    ...
    print(f"  [IsoForest] Flagged {n_outliers} records as outliers ({n_outliers/len(X):.2%}).")
    
    # 3. Protect Failures (Class 1) if y is provided
    if y is not None:
        n_pos_flagged = ((y == 1) & (mask == False)).sum()
        if n_pos_flagged > 0:
            print(f"  [Safety] Rescuing {n_pos_flagged} positive class failures from the outlier set.")
            # Force keep all failures (y == 1)
            # We assume ALL failures are valid signals, even if anomalous.
            mask[y == 1] = True
            
    n_final_drops = (~mask).sum()
    print(f"  [Final] Dropping {n_final_drops} outliers (Only Negatives) -> {n_final_drops/len(X):.2%} drop rate.")
...
def analyze_isolation_forest_outliers(X, y_raw, mask):
    ...
    print(f"  [Analysis] Failures inside dropped set: {n_failures} (Failure Rate: {n_failures/n_dropped:.2%})")
    print(f"  [Analysis] Base Failure Rate of Dataset: {y_raw.mean():.2%}")
    
    if (n_failures/n_dropped) > (y_raw.mean() * 5):
        print("  WARNING: Outlier set has highly disproportionate number of failures. Review contamination.")
