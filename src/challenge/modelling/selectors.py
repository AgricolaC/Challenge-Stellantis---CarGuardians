...
class KruskalSelector(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            raise NotImplementedError("KruskalSelector supports pandas DataFrame input only to ensure correct feature mapping.")
...
class ConsensusSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        if X_data.shape[0] > 10000:
            idx = np.random.RandomState(self.random_state).choice(X_data.shape[0], 10000, replace=False)
            X_mi = X_data[idx]
            y_mi = y[idx]
        else:
            X_mi, y_mi = X_data, y
        ...
        mi_scores = mutual_info_classif(X_mi_filled, y_mi, discrete_features='auto', 
                                        random_state=self.random_state, n_neighbors=3)
        mi_indices = np.argsort(mi_scores)[-self.top_n:] 
        
        # --- VOTER 3: LightGBM (Interactions) ---
        lgbm = lgb.LGBMClassifier(n_estimators=100, n_jobs=self.n_jobs, 
        ...
        if len(self.selected_indices_) == 0:
            ...
            norm_imp = lgbm.feature_importances_ / (lgbm.feature_importances_.max() + 1e-9)
            hybrid_score = votes + norm_imp
            self.selected_indices_ = np.argsort(hybrid_score)[-self.top_n:]

        print(f"ConsensusSelector: Selected {len(self.selected_indices_)} features from {n_features} total.")
