...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../challenge"))
)
...
def test_missingness_clustering():
    ...
    preprocessor = ScaniaPreprocessor(
        reduce_missingness=True, cluster_threshold=0.15, missing_flag_thresh=0.01
    )
    ...
    if hasattr(preprocessor, "missingness_cluster_map_"):
        ...
        for cols in preprocessor.missingness_cluster_map_.values():
            if set(
                ["col_0_is_missing", "col_1_is_missing", "col_2_is_missing"]
            ).issubset(set(cols)):
                found_A = True
                break
